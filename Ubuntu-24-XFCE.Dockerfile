...
RUN apt-get update && \
    ...
    if [ "$BUILD_KDE" = "conc" ]; then \
        apt-get install -y --no-install-recommends \
            ...
            catfish mugshot xcursor-themes dmz-cursor-theme xfce4-clipman-plugin xinit xorg dbus-x11 at-spi2-core \
            tumbler fonts-lklug-sinhala adwaita-icon-theme-full hicolor-icon-theme gnome-icon-theme tango-icon-theme \
            gtk2-engines-murrine gtk2-engines-pixbuf arc-theme numix-gtk-theme materia-gtk-theme papirus-icon-theme \
...
ExecStart=/bin/bash -lc 'DISPLAY=:5 startxfce4'
Restart=no
RestartSec=3
