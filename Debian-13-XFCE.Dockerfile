...
RUN apt-get update && \
    ...
    if [ "$BUILD_XFCE" = "conc" ]; then \
        apt-get install -y --no-install-recommends \
        xfce4 desktop-base xfce4-terminal xfce4-session xfce4-goodies xfce4-taskmanager mousepad \
        galculator nemo-fileroller ristretto xfce4-screenshooter catfish xcursor-themes xfce4-clipman-plugin \
        xinit xorg dbus-x11 at-spi2-core tumbler adwaita-icon-theme-full hicolor-icon-theme gnome-icon-theme \
